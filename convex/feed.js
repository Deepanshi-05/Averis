...
export default function FeedPage() {
  ...
  // ✅ Convex native queries (NO custom hooks)
  const feedData = useQuery(api.feed.getFeed, { limit: 15 });
  const suggestedUsers = useQuery(api.feed.getSuggestedUsers, { limit: 6 });
  const trendingPosts = useQuery(api.feed.getTrendingPosts, { limit: 15 });
  ...
  const isLoading =
    feedData === undefined ||
    (activeTab === "trending" && trendingPosts === undefined);
